...
if [[ $SEARCH == *@* ]] || [[ $PACKAGE == *@* ]]; then
    ...
    while IFS= read -r URL; do
        ...
        if [[ $URLNAME == "$REPONAME" ]]; then
            ...
            if ((LEN == 0)) || [[ -z ${_LEN[*]} ]]; then
                if [[ -n $SEARCH ]]; then
                    fancy_message warn $"There is no package with the name %b in the repo %b" "$IRed${SEARCH%%@*}$NC" "$CYAN$REPONAME$NC"
                else
                    fancy_message warn $"There is no package with the name %b in the repo %b" "$IRed${PACKAGE%%@*}$NC" "$CYAN$REPONAME$NC"
                fi
            ...
            if [[ -n $SEARCH ]]; then
                ...
                if [[ -z ${INFORESULTS[*]} ]]; then
                    fancy_message error $"There is no package with the name %b in the repo %b" "$IRed${INFOQUERY%%@*}$NC" "$CYAN$REPONAME$NC"
    ...
    fancy_message warn $"%b is not on your repo list or does not exist" "$IRed$REPONAME$NC"
    error_log 3 "search $PACKAGE@$REPONAME"
    exit 1
fi
...
if [[ -n ${SEARCH} ]]; then
    ...
    if ((LEN == 0)) || [[ -z ${searchout[*]} ]]; then
        fancy_message warn $"There is no package with the name %b" "$IRed${SEARCH%%@*}$NC"
        exit 1
    fi
...
while IFS= read -r URL; do
    ...
    if [[ ${URL} == "/"* ]] || [[ ${URL} == "~"* ]] || [[ ${URL} == "."* ]]; then
        sed -i "s#${URL}#file://$(readlink -f ${URL})#g" "$SCRIPTDIR/repo/pacstallrepo" 2> /dev/null \
            || fancy_message warn $"Add %s to the local repo absolute path on %b" "\"file://\"" "\e]8;;file://$SCRIPTDIR/repo/pacstallrepo\a$CYAN$SCRIPTDIR/repo/pacstallrepo$NC\e]8;;\a"
        URL="file://$(readlink -f ${URL})"
    elif [[ ${URL} == "file://"* && ${URL} == *"/~/"* ]]; then
        sed -i "s#${URL}#${URL/'~'/$HOME}#g" "$SCRIPTDIR/repo/pacstallrepo" 2> /dev/null \
            || fancy_message warn $"Replace '~' with the full home path on %b" "\e]8;;file://$SCRIPTDIR/repo/pacstallrepo\a$CYAN$SCRIPTDIR/repo/pacstallrepo$NC\e]8;;\a"
        URL="${URL/'~'/$HOME}"
    fi
    URL="$(repo.format "${URL}")"
    if ((REPOMSG!=1)); then
        if [[ -z ${URL} ]]; then
            fancy_message error $"Pacstall repo line improperly formatted: %b" "${CYAN}${URL}${NC}"
...
if ((LEN == 0)) || [[ -z ${_LEN[*]} ]]; then
    fancy_message error $"There is no package with the name %b" "$IRed$PACKAGE$NC"
...
# check if we are looking at info
    ...
    if [[ -z ${INFORESULTS[*]} ]]; then
        fancy_message error $"There is no package with the name %b" "$IRed$INFOQUERY$NC"
        error_log 3 "search $INFOQUERY"
        exit 1
    fi
    for inres in "${INFORESULTS[@]}"; do
...
# Variable $type used for the prompt
    ...
    if ((LEN == 1)); then
        ...
        if [[ -n $PACSTALLREPO ]]; then
            ...
            ask $"%bDo you want to %s %b from the official repo?" "\e[1A\e[K" "$type" "$GREEN${PACKAGELIST[$IDX]}$NC" Y
            if ((answer == 1)); then
        ...
        for IDX in "${_LEN[@]}"; do
            ...
            ask $"%bDo you want to %s %b from the repo %b?" "\e[1A\e[K" "$type" "$GREEN${PACKAGELIST[$IDX]}$NC" "$CYAN${searchedrepo}$NC" Y
            unset searchedrepo
            if ((answer == 1)); then
