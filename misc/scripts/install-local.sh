...
mkdir -p "$SRCDIR"
cd "$SRCDIR" 2> /dev/null || ( fancy_message warn "Could not enter ${SRCDIR}" && exit 1)
...
case "$url" in
	*.git)
		...
		cd ./*/ 2> /dev/null || ( fancy_message warn "Could not enter into the cloned git repository" && exit 1)
	...
	*.zip)
		...
		# cd into it
		cd ./*/ 2> /dev/null || ( fancy_message warn "Could not enter into the downloaded archive" && exit 1)
		# export srcdir
		export srcdir="/tmp/pacstall/$PWD"
		# Make the directory available for users
		sudo chown -R "$LOGNAME":"$LOGNAME" . 2>/dev/null
	...
	*.deb)
		...
		if sudo apt install -y -f ./"${url##*/}" 2>/dev/null; then
			...
			cd "$DIR" 2> /dev/null || ( fancy_message warn "Could not enter into ${DIR}" && exit 1)
...
cd "$HOME" 2> /dev/null || ( fancy_message warn "Could not enter into ${HOME}" && exit 1)
...
cd /usr/src/pacstall 2> /dev/null || ( fancy_message warn "Could not enter into ${STOWDIR}" && exit 1)
