...
if ! is_package_installed "${name}"; then
    ...
    if [[ -n $replace ]]; then
		...
		if [[ "$(dpkg-query -W -f='${Status}' "$replace" 2> /dev/null)" =~ ^(install ok installed|ok unpacked) ]]; then
...
if [[ -n ${build_depends[*]} ]]; then
    ...
    for build_dep in "${build_depends[@]}"; do
		if [[ "$(dpkg-query -W -f='${Status}' "${build_dep}" 2> /dev/null)" =~ ^(install ok installed|ok unpacked) ]]; then
