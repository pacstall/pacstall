...
function makedeb() {
    ...
    local estsize
    estsize="$( du -s --apparent-size --exclude=DEBIAN -- "$STAGEDIR/$pacname" | cut -d$'\t' -f1)"
...
function install_deb() {
    ...
    if ((PACSTALL_INSTALL != 0)); then
        ...
        if ! apt-get install --reinstall "$STAGEDIR/$debname.deb" -y --allow-downgrades 2> /dev/null; then
            echo -ne "\t"
        ...
        echo -e "Package: ${provides[*]}\nPin: release o=*\nPin-Priority: -1\n" |  tee "/etc/apt/preferences.d/${pacname//./-}-pin" > /dev/null
