...
if [[ -z $number || -z $inst ]]; then
    fancy_message error $"'%s' and '%s' cannot be empty!" "number" "package"
    fancy_message sub $"use the syntax: %b" "-Qa ${GREEN}package${BYellow}#${YELLOW}NUM${NC}(${BPurple}@${PURPLE}metalink${NC})"
    exit 1
fi
